...
const SlideOutPanel: FC<SlideOutPanelProps> = ({ isOpen, onClose, setNewPost, handlePostChange }) => {  
    ...
    // TODO: Refactor this to use a single useEventsData hook.
    ...
    const renderImages = (event: Event) => {
        ...
        const dateString = eventTime.toLocaleDateString('en-GB', { month: '2-digit', day: '2-digit' });
        const dateTimeString = `${dateString} ${milTime}`;
    
        const oneHourBeforeEventTime = new Date(eventTime.getTime() - 60 * 60 * 1000);
        ...
        if (oneHourBeforeEventTime > currentTime) {
            ...
            displayElement = (
                <span className="ml-3 text-sm text-lightPurple font-semibold">{'Kick Off:'}</span>
            );
        } else {
            displayElement = (
                <div className="ml-1">
